...
})({"etmzB":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
...
fetch(url).then(parsedJson).then((parsedResponse)=>{
    const urls = parsedResponse.results.map((result)=>result.url
    );
    const fetches = urls.map((url1)=>fetch(url1).then((response)=>response.json()
        )
    );
...
},{}]},["etmzB","h7u1C"], "h7u1C", "parcelRequire82b7")
